...
Function Restart-Host {
	...
	If ($ComputerName) {
		...
		Test-TCPPort -ComputerName $ComputerName -SubScripts $SubScripts -Port '135' -Timeout '120'
		If ($global:TestTCPPort.PortOpen -eq $true) {
			[boolean]$rpc135 = $true
		}
		Test-TCPPort -ComputerName $ComputerName -SubScripts $SubScripts -Port '445' -Timeout '120'
		If ($global:TestTCPPort.PortOpen -eq $true) {
			[boolean]$rpc445 = $true
		}
