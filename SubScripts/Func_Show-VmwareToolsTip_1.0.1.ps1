...
Function Show-VmwareToolsTip {
	...
	#region Tips
		...
		$c = {
			...
			Write-Host 'parameter for Add-NFSDS can be used to input multiple hosts from the command line.'
			Write-Host ''
			Write-Host '  EXAMPLE: ' -NoNewline
			Write-Host 'Add-NFSDS -List server01,server02,server03' -ForegroundColor Yellow -NoNewline
			Write-Host ' <enter>'
			Write-Host ''
		}
		...
		$d = {
			...
			Write-Host 'The ' -NoNewline
			Write-Host '-List ' -ForegroundColor Yellow -NoNewline
			Write-host 'parameter for Add-NFSDS also excepts a PowerShell array variable to input multiple hosts from the command line.'
			Write-Host ''
			Write-Host '  EXAMPLE: ' -NoNewline
			Write-Host '$mylist = @("server01","server02","server03")' -ForegroundColor Yellow -NoNewline
			Write-Host ' <enter>'
			Write-Host '           Add-NFSDS -List $mylist' -ForegroundColor Yellow -NoNewline
			Write-Host ' <enter>'
			Write-Host ''
		}
		...
		$e = {
			...
			Write-Host 'switch for Add-NFSDS can be used to skip the Out-GridView Results (Spreadsheet Poppup) at the end.'
			Write-Host ''
			Write-Host '  EXAMPLE: ' -NoNewline
			Write-Host 'Add-NFSDS -FileName hostlist01.txt -SkipOutGrid' -ForegroundColor Yellow -NoNewline
			Write-Host ' <enter>'
			Write-Host ''
		}
		...
		$f = {
			...
			Write-Host 'switch for Add-NFSDS can be used to throttle the background jobs. The default value is high so if system resources start to pose an issue use this to set a lower number of jobs that can run simultaneously.'
			Write-Host ''
			Write-Host '  EXAMPLE: ' -NoNewline
			Write-Host 'Add-NFSDS -FileName hostlist01.txt -MaxJobs 50' -ForegroundColor Yellow -NoNewline
			Write-Host ' <enter>'
			Write-Host ''
		}
